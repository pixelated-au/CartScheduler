...
test('show users with no overlapping shifts', function () {
    ...
    /** @var Collection<User> $attachedUsers */
    $attachedUsers = $users->get(0);

    /** @var Collection<User> $unattachedUsers */
    ...
    foreach ($showOnlyAvailableValues as $showOnlyAvailable) {
        ...
        expect($availableUsers)->toHaveCount($unattachedUsers->count());

        /** @var User $availableUser */
        foreach ($availableUsers as $availableUser) {
            expect($unattachedUsers->contains(fn (User $user) => $user->getKey() === $availableUser->getKey()))->toBeTrue();
        }
    }
});

test('show users who are available on day and hour and arnt on vacation', function () {
    ...
    foreach ($showOnlyAvailableValues as $showOnlyAvailable) {
        ...
        expect($availableUsers)->toHaveCount($showOnlyAvailable ? 3 : $users->count());

        foreach ($chosenUsers as $chosenUser) {
            expect($availableUsers->contains(fn (User $user) => $user->getKey() === $chosenUser->getKey()))->toBeTrue();
        }
    }
});

test('show only responsible brothers', function () {
    $locations = Location::factory()
        ->state(['max_volunteers' => 3])
        ->count(2)
        ->has(Shift::factory()->everyDay9am())
    ...
    /** @var Collection<int, Collection<int, User>> $responsibleBros */
    $responsibleBros = User::factory()->responsibleBrother()->count(3)->create()->chunk(2);

    $attachedUsers = $users->get(1)->merge($responsibleBros->get(1));
    // 3 users, 1 of which is a responsible brother
    $remainingUsers = $users->get(0)->merge($responsibleBros->get(0));

    // 5 users, 2 of which are responsible brothers
    $dateRange = collect();
    ...
    expect($availableUsers->filter(fn (User $user) => $user->responsible_brother === true))->toHaveCount($availableUsers->count());

    /** @var User $availableUser */
    foreach ($availableUsers as $availableUser) {
        expect($remainingUsers->contains(fn (User $user) => $user->getKey() === $availableUser->getKey()))->toBeTrue();
    }
});

test('show only pioneers', function () {
    $locations = Location::factory()
        ->state(['max_volunteers' => 3])
        ->count(2)
        ->has(Shift::factory()->everyDay9am())
    ...
    /** @var Collection<int, Collection<int, User>> $publishers */
    $publishers = User::factory()->count(5)->publisher()->create()->chunk(3);

    /** @var Collection<int, Collection<int, User>> $pioneers */
    $pioneers = User::factory()->pioneer()->count(3)->create()->chunk(2);

    $attachedUsers = $publishers->get(1)->merge($pioneers->get(1));
    // 3 users, 1 of which is a pioneer
    $remainingUsers = $publishers->get(0)->merge($pioneers->get(0));

    // 5 users, 2 of which are pioneers
    $dateRange = collect();
    $attachedUsers->each(fn (User $user) => $dateRange
        ->push([
            'shift_date' => '2023-05-15',
    ...
    expect($availableUsers->filter(fn (User $user) => $user->serving_as !== 'publisher'))->toHaveCount($availableUsers->count());
...
test('show only elders', function () {
    $locations = Location::factory()
        ->state(['max_volunteers' => 3])
        ->count(2)
        ->has(Shift::factory()->everyDay9am())
    ...
    /** @var Collection<int, Collection<int, User>> $elders */
    $elders = User::factory()->count(5)->elder()->create()->chunk(3);

    /** @var Collection<int, Collection<int, User>> $nonElders */
    $nonElders = User::factory()->enabled()->state(['appointment' => null])->count(3)->create()->chunk(2);

    $attachedUsers = $elders->get(1)->merge($nonElders->get(1));
    // 3 users, 1 of which is an elder
    $remainingUsers = $elders->get(0)->merge($nonElders->get(0));

    // 5 users, 2 of which are elders
    $dateRange = collect();
    $attachedUsers->each(fn (User $user) => $dateRange
        ->push([
            'shift_date' => '2023-05-15',
    ...
    expect($availableUsers->filter(fn (User $user) => $user->appointment === 'elder'))->toHaveCount($availableUsers->count());
...
test('show only ministerial servants', function () {
    $locations = Location::factory()
        ->state(['max_volunteers' => 3])
        ->count(2)
        ->has(Shift::factory()->everyDay9am())
    ...
    /** @var Collection<int, Collection<int, User>> $mSs */
    $mSs = User::factory()->count(5)->ministerialServant()->create()->chunk(3);

    /** @var Collection<int, Collection<int, User>> $nonMSs */
    $nonMSs = User::factory()->enabled()->state(['appointment' => null])->count(3)->create()->chunk(2);

    $attachedUsers = $mSs->get(1)->merge($nonMSs->get(1));
    // 3 users, 1 of which is an MS
    $remainingUsers = $mSs->get(0)->merge($nonMSs->get(0));

    // 5 users, 2 of which are MSs
    $dateRange = collect();
    $attachedUsers->each(fn (User $user) => $dateRange
        ->push([
            'shift_date' => '2023-05-15',
    ...
    expect($availableUsers->filter(fn (User $user) => $user->appointment === 'ministerial servant'))->toHaveCount($availableUsers->count());
...
test('show only appointed brothers', function () {
    $locations = Location::factory()
    ...
    /** @var Collection<int, Collection<int, User>> $appointedBros */
    ...
    /** @var Collection<int, Collection<int, User>> $nonAppointed */
    ...
    $attachedUsers = $appointedBros->get(1)->merge($nonAppointed->get(1));
    // 4 users, 3 of which are appointed brothers
    $remainingUsers = $appointedBros->get(0)->merge($nonAppointed->get(0));

    // 5 users, 3 of which are appointed brothers
    $dateRange = collect();
    $attachedUsers->each(fn (User $user) => $dateRange
        ->push([
            'shift_date' => '2023-05-15',
    ...
    expect($availableUsers->filter(
        fn (User $user) => $user->appointment === 'elder' || $user->appointment === 'ministerial servant'))->toHaveCount($availableUsers->count());
...
test('toggle all user filters', function () {
    $locations = Location::factory()
    ...
    /** @var Collection<int, Collection<int, User>> $nonRegular */
    ...
    /** @var Collection<int, Collection<int, User>> $regular */
    ...
    $attachedUsers = $nonRegular->get(1)->merge($regular->get(1));
    // 4 users, 3 of which are appointed brothers
    $remainingUsers = $nonRegular->get(0)->merge($regular->get(0));
    ...
    ))->toHaveCount($availableUsers->count());
