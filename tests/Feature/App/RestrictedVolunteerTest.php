...
test('restricted user cannot reserve', function () {
...
test('user cannot release', function () {
    $user = User::factory()->enabled()->state(['is_unrestricted' => false])->create();
...
test('restricted user cannot see shifts outside of their own roster', function () {
    $users = User::factory()
        ->enabled()
        ->count(4)
