...
<template>
    <div
        class="col-span-full bg-gray-100 dark:bg-slate-700 p-6 rounded-lg shadow-lg grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        <div>
        ...
        <div v-if="allTags?.length" class="px-3">
            <SlickList axis="xy" v-model:list="allTags">
                <SlickItem v-for="(tag, i) in allTags"
                    ...
                    <button
                        class="px-2 py-1 focus:outline-none dark:text-gray-100 focus:border-gray-900 dark:focus:border-slate-200 focus:ring focus:ring-gray-300 dark:focus:ring-gray-700 leading-none hover:bg-gray-600 hover:text-gray-100 active:bg-gray-900 dark:hover:bg-slate-400 dark:hover:text-gray-800 dark:active:bg-gray-200"
                        @click="selectTag(tag)">
                        {{ tag.name }}
                    </button>
                </SlickItem>
